...
class Estor(nn.Module):
    def __init__(self, config):
        ...
        # 双向LSTM层
        self.lstm = nn.LSTM(hidden_size, hidden_size // 2, num_layers=1, bidirectional=True, batch_first=True)
    ...
    def forward(self, word_embedding,attention_mask, tag_to_spans):  # tag_to_span是一个加了batch的dict
        ...
        banben = 'lite'
        assert banben in ['lite','encoder','bilstm']
        if banben == 'lite':
            output = output_embedding
        elif banben == 'bilstm':
            output,_ = self.lstm(output_embedding)
        else:
