...
TRAINED_DIR=$CURRENT_DIR/outputs/ecommerce_output/robert_pos_add_crf/checkpoint-3340
DATA_DIR=$CURRENT_DIR/datasets
OUTPUR_DIR=$CURRENT_DIR/outputs
TASK_NAME="ecommerce"
#
python main.py \
  --backbone=bert_estor-att-add_crf \
  ...
  --local_rank=-1 \
  --num_train_epochs=50 \
  --freeze_backbone_epoch=30\
  --tagging_rate=1\
  --enumerate_mode=attention\
  --gate_scaling_rate=0.6\
  --gate_dropout_rate=0.5\
  --if_add_a_self_attention=0\
  --markup=bio\
  --if_merge_by_add=1\
